...
export function CreateProposalModal({ isOpen, onClose }: CreateProposalModalProps) {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    const emoji = selectedEmoji || getAvailableEmoji(usedEmojis);
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center justify-center w-16 h-16 border-2 border-gray-300 rounded-lg text-3xl">
              {selectedEmoji || getAvailableEmoji(usedEmojis)}
            </div>
            <button
              type="button"
          ...
          {showEmojiPicker && (
            <div className="mt-3 p-3 border border-gray-200 rounded-lg bg-gray-50 max-h-48 overflow-y-auto">
